...
#k1 loop parameters
lower_index_bound1=5  #lower bound of the for loop
upper_index_bound1=40 #upper bound of the for loop
offset1=0             #offset of the variable we want to iterate
div1=1000               #controls the step size of the variable when we iterate it (=2 -> /2)

#k2 loop parameters
lower_index_bound2=0  #lower bound of the for loop
upper_index_bound2=50 #upper bound of the for loop
offset2=0             #offset of the variable we want to iterate
div2=50               #controls the step size of the variable when we iterate it (=2 -> /2)

for index in $(#vary $index from (lower_index_bound) to (upper_index_bound)
    seq $lower_index_bound $upper_index_bound
); do
    ...
    for index1 in $(seq $lower_index_bound1 $upper_index_bound1); do
        ...
        for index2 in $(seq $lower_index_bound2 $upper_index_bound2); do
            ...
            for fwav in pitch_db/train/*.wav; do
                ff0=${fwav/.wav/.f0}
                #
                $GETF0 $fwav $ff0 -t $k1 -a $k2 >/dev/null || (
                    echo "Error in $GETF0 $fwav $ff0"
                    exit 1
                )
            done

            res=$($EVAL -l pitch_db/train/*f0ref || (
                echo "Error in $GETF0 $fwav $ff0"
                exit 1
            ))
            echo "$k1 $k2 $res"
